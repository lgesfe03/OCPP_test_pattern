...
async def WebSocketserver_send(websocket, path):
    while (1):
        ...
        count = count_time()
        # if count == 8:
        #     await websocket.send(OCPP_CALL_SendLocalList1)
        # elif count == 7:
        #     await websocket.send(OCPP_CALL_SendLocalList2)
        # elif count == 6:
        #     await websocket.send(OCPP_CALL_SendLocalList3)
        # elif count == 5:
        #     await websocket.send(OCPP_CALL_SetChargingProfile1)
        # elif count == 4:
        #     await websocket.send(OCPP_CALL_SetChargingProfile2)
        if count == 3:
            await websocket.send(OCPP_CALL_GetCompositeSchedule2)
        elif count == 2:
            await websocket.send(OCPP_CALL_GetCompositeSchedule3)
        elif count == 1:
            await websocket.send(OCPP_CALL_SetChargingProfile2)
        else:
...
async def WebSocketserver_ChangeConfiguration(websocket, path):
    while (1):
        count = count_time()
        if count == 2:
            await websocket.send(OCPP_CALL_ChangeConfiguration3)
        elif count == 1:
            await websocket.send(OCPP_CALL_ChangeConfiguration2)
        else:
            await websocket.send(OCPP_CALL_ChangeConfiguration1)
        Rxdata = await websocket.recv()
        if "Authorize" in Rxdata:
            await websocket.send(OCPP_CallResult_authorize)
        elif "BootNotification" in Rxdata:
            tx = replace_uid_time(Rxdata,OCPP_CallResult_boot_r)
            await websocket.send(tx)
...
async def WebSocketserver_GetCompositeSchedule(websocket, path):
    while (1):
        ...
        if "Authorize" in Rxdata:
            ...
            tx = replace_uid_time(Rxdata,OCPP_CallResult_heart_r)
            await websocket.send(tx)
            await websocket.send(OCPP_CALL_GetCompositeSchedule1)
        else:
            print("=========skip========")
#5-08
async def WebSocketserver_GetConfiguration(websocket, path):
    while (1):
        ...
        if "Authorize" in Rxdata:
            ...
            count = count_time()
            if count == 2:
                await websocket.send(OCPP_CALL_GetConfiguration3)
            elif count == 1:
                await websocket.send(OCPP_CALL_GetConfiguration2)
            else:
                await websocket.send(OCPP_CALL_GetConfiguration1)
        else:
            print("=========skip========")
#5-15
...
async def WebSocketserver_SetChargingProfile(websocket, path):
    while (1):
        #await websocket.send(OCPP_Call_ChangeAvailable)
        Rxdata = await websocket.recv()
...
start_server = websockets.serve(WebSocketserver_send, Host, SocketPort,compression=None,ping_interval=5)
